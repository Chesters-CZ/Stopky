...
public class Stopky {
    ...
    JTable tebel = new JTable(new DefaultTableModel(new String[][]{
            {"Bertík", ""},
            {"Žertík", ""},
            {"Alfons", ""}
    ...
    public void funguj() {
        ...
        tebel.setBounds(20, 30, 544, 228);
        tebel.setDefaultRenderer(Object.class, new TentokratChciTlustejPrvniSloupec());
        tebel.setBorder(new LineBorder(new Color(0, 0, 0), 2));
        tebel.setRowHeight(76);
        prepare(tebel);
    ...
    public void clickityClickIsThatADick(ActionEvent e) {
        ...
        if (startOrLap.equals(source) && startOrLap.isEnabled()) {
            if (!bezi) {
                ...
                tebel.setValueAt(getTime(), finishers++, 1);
                if (finishers == 3) {
                    bezi = false;
                    startOrLap.setEnabled(false);
                    stopOrReset.setText("Reset");
                    timer.setText(tebel.getValueAt(finishers - 1, 1).toString());
                }
            }
